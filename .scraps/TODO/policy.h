 ...
 */
template <typename Action> struct policy::ExtActionT {
  explicit policy::ExtActionT(size_t idx) : action{idx} {}
...
struct Policy_UCB {
  ...
  Action operator()(const std::vector<policy::ExtActionT<Action>> &actions) const;
 ...
 */
template <typename Action>
double SampleAverage(const policy::ExtActionT<Action> &e_action) {
...
double UCB(const double c, int total_visits,
           const policy::ExtActionT<Action> &e_action) {
...
template <typename Action>
Action
Policy_UCB::operator()(const std::vector<policy::ExtActionT<Action>> &actions) const {
  ...
  return *std::max_element(
      ...
      [&](const policy::ExtActionT<Action> &a, const policy::ExtActionT<Action> &b) {
...
Action Policy_Greedy::operator()(
    const std::vector<policy::ExtActionT<Action>> &actions) const {
  assert(not actions.empty() && "Agent has empty actions buffer");
