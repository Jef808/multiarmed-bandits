...
function getYExtent(data: Arm[]): NumberBounds {
  return data
    .map(
      (arm) =>
        d3.extent(arm.samples().map((s: Sample) => s.value)) as NumberBounds
    )
    .reduce(
      (accMinMax, currentMinMax) => {
...
function yBinData(
  ...
  return new Map<ArmIdType, SampleBin[]>(
    data.map((a) => [a.id(), bins(a.samples())])
  );
}

export function recentStepsColorMap(data: Arm[], recentRatio: number = 0.25) {
  const totalSamples = data.reduce(
    (total, arm) => total + arm.samples().length,
    0
  );
  ...
  const entries: [ArmIdType, CMValue][] = data.map((arm) => {
    return [
      arm.id(),
      scaleDiv(
        arm.samples().filter((s) => s.id >= totalSamples - recentSamples)
          .length - uniformMeanExp
      ),
    ];
  });
...
export function violinSvgPath(
  ...
  return new Map<ArmIdType, SVGPathType>(
    binIds.map((armId, idx) => [armId, rectToViolin(binValues[idx]) ?? ""])
  );
}
