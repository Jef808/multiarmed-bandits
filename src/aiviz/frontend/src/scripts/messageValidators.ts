    ...
    try {
        ...
        return {
            validation: requiredProps.every(prop => prop in response)
                ...
                ? ValidationResult.Ok
                : ValidationResult.InvalidData : ValidationResult.RequiredPropMissing,
            response
        };
    }
    catch (e) {
        return {
            validation: (e as Error).name === 'SyntaxError'
                ? ValidationResult.InvalidJSON : ValidationResult.UnknownError,
            response: {}
        }
    }
}
